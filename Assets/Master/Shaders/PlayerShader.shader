...
Shader "Shader Forge/PlayerShader" {
    Properties {
        ...
        _node_8086 ("node_8086", Range(0, 50)) = 33.63248
    }
    SubShader {
        Tags {
            "Queue"="AlphaTest"
...
////// Specular:
                ...
                float3 diffuseColor = (((_HighlightColor.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelExp))*_HighlightIntensity)+(_MainTex_var.rgb*_Color.rgb)); // Need this for specular when using metallic
...
/////// Diffuse:
                ...
                float3 backLight = max(float3(0.0,0.0,0.0), -NdotLWrap + w ) * float3(_node_8086,_node_8086,_node_8086);
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 diffColor = (((_HighlightColor.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelExp))*_HighlightIntensity)+(_MainTex_var.rgb*_Color.rgb));
                float specularMonochrome;
                float3 specColor;
